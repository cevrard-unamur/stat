# This package contains useful function to calculate hypothesis tests
...
test.base <- function(t, df, pfunc, qfunc, vars, risk.level=0.05, alternative=c("two.sided", "less", "greater")) {
  ...
  cat(sprintf("df = (%s), p-value = %.5f\n", paste(df,collapse=","), p.value))
  cat(sprintf("IC = [%.5f; %.5f]", e1, e2))
...
test.compare_means.base.sd <- function(n_x, mean_x, var_x, n_y, mean_y, var_y, risk.level=0.05, alternative="two.sided") {
  var_x = var_x^2
  var_y = var_y^2
...
test.equal_distributions.normal <- function (categories, values) {
  frequences = values / sum(values)
  centres = categories - .5 # on prend le centre des classes
  m = sum(frequences * centres) # définition de l'espérance
  s = sqrt(sum(frequences * (centres - m)^2)) # définition de l'écart-type

  frequences_norm = pnorm(categories, m, s) - pnorm(categories-1, m, s)

  test.equal_distributions(values, frequences_norm)
}
